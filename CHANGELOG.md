...
- `http/repeater` - Repeat one creates a RequestRepeater that can execute the original HTTP request multiple times with configurable options for delay, backoff, and parallel execution.
- `clone()` method to `http` package.
